...
import image from '../../image/allcine5.svg'
...
import CanalBtn from "../CanalBtn";
import ShapShapBtn from "../ShapShapBtn";
...
export default function Sidbare({content}) {
    ...
    useEffect(() => {
        async function fetchData() {
            await MyRequest('alertCount', 'GET', {}, { 'Content-Type': 'application/json' })
                .then(async (response) => {
    ...
    return (
        <Drawer variant="permanent" open={open}>
            ...
            >
                <Image sx={{backgroundColor:"red"}} src={image} width={150} height={60}
                />
                ...
                {user===2?
                    <ListItem key={1} disablePadding sx={{display: 'block'}}>
                        ...
                        <ButtonSide text={"Ventes des articles"} open={open} icon={<PaidIcon/>}
                                    link={"/articles/vente"}/>
                        <ButtonSide text={"faire une Reservation"} open={open} icon={<PaidIcon/>}
                        ...
                        }} onClick={()=>router.push("/articles/alert")}><ListItemIcon sx={{
                            color: "white",
                            minWidth: 0,
                            mr: 1,
                            justifyContent: 'center',
                        }}><Typography component="h3" sx={{fontSize: 23,color:"white"}} variant="h5">
                            {data.alert}
                        </Typography>
                        </ListItemIcon><ListItemText sx={{marginLeft:1}} primary={"Alert"} /></ListItemButton>:null}
