...
import FaireCanal from "./FaireCanal";
...
function EnhancedTableToolbar(props) {
    ...
    const removeSelect = async () => {
        setLoading(true)
        await MyRequest('ventecanals/1',   'DELETE', {"data":data}, { 'Content-Type':'application/json' })
            .then(async (response) => {
...
export default function FaireCanalTable() {
    const {user,setUser}=useContext(UserContext)
    ...
    useEffect(() => {
        const fetchData = async () => {
            await MyRequest('ventecanals', 'GET', {}, { 'Content-Type': 'application/json' })
                .then((response) => {
